...
This notebook provides a guide to the workflow of a basic _optimal power flow_ (OPF) problem (_OPF LV20 26bus_)
"""
...
# ╔═╡ d83630df-9306-45dc-9602-d7988ef49723
...
case_path = "E:\\lvdata\\LV20_26bus_Fr_Copy"
...
# ╔═╡ 97701ba8-697f-4bb1-bea9-8b3c0f02109e
md"""
...
# ╔═╡ b9607452-d6a6-4f47-98f7-bd564af30d52
dss(odssstring)

# ╔═╡ 9b321fe9-5008-4360-be4e-c8fda0df7f22
busdistances = ODSS.Circuit.AllBusDistances() # returns the distance from each bus to parent EnergyMeter, so I added energymeter to the original master file

# ╔═╡ 68b47c0c-0c87-49bb-8e01-5f3d2804f65b
busnames = ODSS.Circuit.AllBusNames()

# ╔═╡ b077de36-e145-4866-ba80-c6d84058458e
busdistancesdf = DataFrame(Bus=busnames, busdistances=busdistances)

# ╔═╡ d307f5a3-d073-4428-8c4d-fcd72c17692c
sort!(busdistancesdf, [:busdistances])  # 2157, 2824 are xfm pri/sec

# ╔═╡ 03d72972-53bd-44e7-9e33-3c93374d6078
busdistancesdf[busdistancesdf.Bus .=="b1699", :busdistances]

# ╔═╡ 1efc242b-1ea7-4a5e-af19-d80cfc6f7308
# busdist_dict = Dict(busdistancesdf.Bus[i] => busdistancesdf.busdistances[i] for i in 1:nrow(busdistancesdf))

# ╔═╡ 8333939e-f85b-49db-9524-9f27cafdbafb
# busdist_dict["b389"]

# ╔═╡ adc2490f-bd41-4d62-94e3-5326a3b0457d
CSV.write("budistances.csv", busdistancesdf)

# ╔═╡ b3d322ed-58fc-4288-9f1d-e80e74e66a25
voltages_file = "E:\\lvdata\\LV20_26bus_Fr_Copy\\lv20_26bus_EXP_VOLTAGES.csv"
...
# ╔═╡ f68ee2c7-4a3c-4009-9683-cb4e10df14e4
# begin   
# 	∠(x) = cis(deg2rad(x))
# 	vpn(pm,pa,nm,na) = hypot(pm.*∠(pa) - nm.*∠(na))  # method 1
# end

# ╔═╡ c3947a01-c317-4f36-80e8-42d4adbc14f6
# begin
...
# ╔═╡ 7ed1648c-a766-48ee-bfb5-e53a72711119
vpn1(pm,pa,nm,na) = hypot(pm*exp(im*pa/180*pi) - nm*exp(im*na/180*pi)) # method 2

# ╔═╡ a14d2e1a-dcff-461b-a0e2-048132229b93
begin
...
# ╔═╡ 91de214f-8f3a-4f42-b1f5-d93f80241835
begin
...
# ╔═╡ 3b2c005e-07f0-467c-a463-6f6983e4ca48
md"""
### plot voltage profiles wrt bus distances
"""

# ╔═╡ 3e36171d-73b9-4b86-bdb8-3d3ad522a0e1
begin
...
# ╔═╡ ffb9292e-3fa9-4f42-9e3e-8c4f88947e19
savefig("Vpg_distance.pdf")

# ╔═╡ 17323017-9ca5-46e0-957e-22f78941aa3c
begin
...
# ╔═╡ ade1a766-3d6a-4523-b7f0-52d32a79cac5
savefig("Vpn_distance.pdf")

# ╔═╡ 8f8f0aab-ccd8-4073-b08a-9041205ef56d
begin
...
# ╔═╡ b74b7110-7726-4e10-bf37-757d20688c27
savefig("Vn_distance.pdf")
...
# ╔═╡ b5b6aa23-b1c2-4afb-84ff-a3f1ba91c4cf
@bind loadscale confirm(Slider(0.5:0.5:30; default=30, show_value=true))

# ╔═╡ 8a56d14a-b214-48b6-a2fd-ca27519d2456
loadscale
...
for (d, load) in data_eng4w["load"]
	load["pd_nom"] .*= loadscale
	load["qd_nom"] .*= loadscale
end

# ╔═╡ ed96c235-a345-41e1-9a18-7bc07a8fcaa5
data_eng4w["load"]
...
# ╔═╡ 4e69c8bc-0e4e-4fc1-80cf-3bea0747f3cd
md"""
This is to improve numerical practice of the model. Now it can also be done in OpenDSS. Usually small and okay to ignore.
"""
...
# ╔═╡ 8a95a737-7d6c-497d-9384-97e42274b036
md"""
_ref: Fred considerations and design goals ..._
(point to a network: not as large as you think IEEE PES test feeder 2015 european LV test feeder ;)

[example] (https://github.com/sanderclaeys/DistributionTestCases.jl/blob/master/src/data/lvtestcase/lvtestcase_pmd_template.dss)
reduce the network size and computation time
"""
...
data_eng4w["voltage_source"]["source"]
...
# ╔═╡ 5e36649a-1770-4966-8964-cf009309eb7d
begin
...
data_eng4w["bus"]["b2824"]   # tx bus

# ╔═╡ 0030064f-16f8-48e2-9c80-81a078f10b0a
data_eng4w["bus"]["b2824"]["terminals"]
...
sol_math4w = res["solution"]

# ╔═╡ 6a46c79b-feaf-4619-afcf-03c584c77c36
sol_math4w["bus"]

# ╔═╡ a0438364-40af-48d0-956b-1bbfbd379095
md"""
transformers can be decomposed into different buses
"""
...
# ╔═╡ afabb831-5f7e-4015-bbe7-84d65636150f
sol_eng4w["bus"]
...
# ╔═╡ 1a0a2070-3319-4abf-ac8a-bb320772d841
md"""
### plot voltage vs busdistances
"""

# ╔═╡ bd34c4bf-fd24-4da1-8030-5e5434c591c1
busids = sort(collect(keys(sol_eng4w["bus"])))

# ╔═╡ c897f02a-ed6e-420b-8cbc-1c30981c8801
vmag_1 = stack([hypot.(sol_eng4w["bus"][bid]["vr"][1:4], sol_eng4w["bus"][bid]["vi"][1:4]) for bid in busids], dims=1)  # size (nbus, 4)

# ╔═╡ 6895275a-268d-4bb3-a78d-7c73750f3ff8
busdistancesdf

# ╔═╡ 43804eec-2298-4da3-b14d-e96ae7323f90
begin
	vmag_1_df = DataFrame(Bus=busids, Va=vmag_1[:,1], Vb=vmag_1[:,2],Vc=vmag_1[:,3], Vn=vmag_1[:,4])
	vmag_1_df_1 = innerjoin(vmag_1_df, busdistancesdf, on=:Bus)
	vmag_1_df_1 = vmag_1_df_1[2:end,:] # exclude xfmr pri
end

# ╔═╡ 52279fb5-b1d8-4613-b574-08ec0541fc90
begin
	scatter(vmag_1_df_1.busdistances, vmag_1_df_1.Va.*1000, label="phase a")
	scatter!(vmag_1_df_1.busdistances, vmag_1_df_1.Vb.*1000, label="phase b")
	scatter!(vmag_1_df_1.busdistances, vmag_1_df_1.Vc.*1000, label="phase c")
	xlabel!("Distance from distribution transformer (km)")
	ylabel!("Vpg (V)")
	ylims!(225, 253)
	# ylims!(248, 250.1)
...
# ╔═╡ 2d82fc7d-0fa5-462d-a6be-30c296dbcdad
begin
	scatter(vmag_1_df_1.busdistances, vmag_1_df_1.Vn.*1000,label="neutral")
	xlabel!("Distance from distribution transformer (km)")
	ylabel!("Vn (V)")
	# ylims!(10,30)
end
...
manifest_format = "2.0"
project_hash = "7883e447164cc3e2846dae7566595224411a828d"
